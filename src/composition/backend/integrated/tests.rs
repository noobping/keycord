use super::entries::git_commit_private_key_requiring_unlock_for_entry;
use super::entries::integrated_entry_backend;
use super::entries::{
    delete_password_entry, password_entry_is_readable, read_password_entry, rename_password_entry,
    save_password_entry,
};
use super::git::git_commit_private_key_requiring_unlock_for_store_recipients as git_commit_private_key_requiring_unlock_for_split_store_recipients;
use super::store::{
...
#[cfg(feature = "fidokey")]
use keycord_fido::{
    reset_shared_native_transport_for_tests, set_shared_native_transport_for_tests,
    FidoAssertion as Fido2AssertionOutput, FidoDeviceLabel as Fido2DeviceLabel,
    FidoEnrollment as Fido2Enrollment, FidoTransport as Fido2Transport,
    FidoTransportError as Fido2TransportError,
};
use keycord_git::has_git_repository;
#[cfg(feature = "fidokey")]
use keycord_keys::generate_fido2_private_key;
#[cfg(all(feature = "fidokey", feature = "fidopin", target_os = "linux"))]
use keycord_keys::set_fido2_security_key_pin;
#[cfg(feature = "hardwarekey")]
use keycord_keys::testing::store_ripasso_hardware_key_bytes;
use keycord_keys::testing::{
    reset_hardware_transport_for_tests, resolved_ripasso_own_fingerprint,
    set_hardware_transport_for_tests,
};
use keycord_keys::{
    ...
    load_available_standard_key_ring, parse_managed_private_key_bytes,
    prepare_managed_private_key_bytes, remove_ripasso_private_key, ripasso_keys_dir,
    ...
    HardwareTransport, HardwareTransportError, ManagedRipassoPrivateKeyProtection,
...
use keycord_keys::{
    ensure_ripasso_private_key_is_ready, PrivateKeyError, PrivateKeyReadinessError,
};
use keycord_preferences::Preferences;
use keycord_stores::integrated_recipients::preferred_ripasso_private_key_fingerprint_for_entry;
use keycord_stores::paths::{recipients_file_for_label, secret_entry_relative_path};
use keycord_stores::recipients::split_store_recipients;
#[cfg(feature = "hardwarekey")]
...
impl HardwareTransport for MockHardwareTransport {
    ...
    fn generate_key_material(
        &self,
        request: &keycord_keys::testing::HardwareKeyGenerationRequest,
    ) -> Result<(DiscoveredHardwareToken, Vec<u8>), HardwareTransportError> {
...
impl HardwareTransport for FailingHardwareTransport {
    ...
    fn generate_key_material(
        &self,
        _request: &keycord_keys::testing::HardwareKeyGenerationRequest,
    ) -> Result<(DiscoveredHardwareToken, Vec<u8>), HardwareTransportError> {
        Err(HardwareTransportError::Other(
...
impl Fido2TransportGuard {
    fn install(transport: Arc<dyn Fido2Transport>) -> Self {
        set_shared_native_transport_for_tests(transport);
        Self
    }
}
...
fn encrypted_private_keys_unlock_for_the_current_session_only() {
    ...
    assert!(matches!(
        ...
        PrivateKeyReadinessError::Locked(_)
    ));
...
fn all_keys_mode_uses_a_nonstandard_layered_entry_format() {
    ...
    let outer_layer = integrated_entry_backend()
        .load_crypto_for_fingerprint(&key_a.fingerprint)
...
fn integrated_backend_uses_the_first_usable_private_key_before_a_coworker_recipient() {
    ...
    assert_eq!(
        integrated_entry_backend()
            .fingerprint_for_label(&store_root, "team/service")
