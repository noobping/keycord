...
use crate::entry_files::{
    is_password_entry_file, label_from_password_entry_path, STANDARD_PASSWORD_ENTRY_EXTENSION,
};
use crate::path_validation::{validated_entry_label_path, validated_relative_directory_path};
use keycord_runtime::secure_fs::write_atomic_file;

pub fn recipients_file_for_relative_dir(
    store_root: &str,
    relative_dir: &str,
...
#[doc(hidden)]
pub fn secret_entry_relative_path(label: &str) -> Result<PathBuf, String> {
...
pub fn existing_entry_file_path(store_root: &str, label: &str) -> Result<Option<PathBuf>, String> {
...
pub fn desired_entry_file_path(store_root: &str, label: &str) -> Result<PathBuf, String> {
    entry_file_path_with_extension(store_root, label)
}

pub fn entry_file_path(store_root: &str, label: &str) -> Result<PathBuf, String> {
...
pub fn recipients_file_for_label(store_root: &str, label: &str) -> Result<PathBuf, String> {
...
pub fn label_from_entry_path(store_root: &Path, entry_path: &Path) -> Result<String, String> {
    label_from_password_entry_path(store_root, entry_path)
        .ok_or_else(|| "Invalid password entry path.".to_string())
}

pub fn ensure_store_directory(store_root: &str) -> Result<PathBuf, String> {
...
pub fn with_updated_recipient_file<T>(
...
pub fn cleanup_empty_store_dirs(store_root: &str, entry_path: &Path) -> Result<(), String> {
...
pub fn collect_password_entry_files(store_root: &Path) -> Result<Vec<PathBuf>, String> {
