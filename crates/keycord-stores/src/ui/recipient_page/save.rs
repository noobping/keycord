...
use crate::recipient_page::{should_refresh_after_save, should_reschedule_after_finish};
use crate::recipients::{split_store_recipients, stores_with_preferred_first};
...
fn maybe_prompt_store_recipients_git_unlock(
    ...
    if !allow_git_unlock_prompt || !state.ports.preferences.uses_integrated_backend() {
        return false;
    }
    ...
    (state.ports.git.prompt_recipient_commit_unlock)(
...
fn maybe_prompt_store_recipients_entry_unlock(
    ...
    let fingerprint = match (state.ports.backend.recipient_unlock_fingerprint)(
    ...
    let stores_list_for_retry = stores_list.clone();
    let state_for_retry = state.clone();
    state.key_management.request_recipient_key_access(
        fingerprint,
        Rc::new(move || {
            save_store_recipients_async(
                &overlay_for_retry,
...
fn save_store_recipients_async(
    ...
    let save_recipients = state.ports.backend.save_recipients.clone();
    spawn_result_task(
        move || {
            save_recipients(
                store_for_thread,
        ...
        move |result| match result {
            Ok(()) => {
                ...
                let store_list_changed = if request.mode.creates_store() {
                    let preferences = &state.ports.preferences;
                    let stores = stores_with_preferred_first(&preferences.stores(), &request.store);
                ...
                if request.mode.creates_store() {
                    rebuild_stores_list(&stores_list, &state, None);
                }
...
mod tests {
    ...
    use crate::recipient_page::{StoreRecipientsMode, StoreRecipientsRequest};
