use crate::paths::recipients_file_for_label;
...
use keycord_keys::{
    ...
    selected_ripasso_own_fingerprint, PrivateKeyReadinessError,
};
        ...
        key_ring
            ...
            .filter(|(_, cert)| {
                cert.userids().any(|user_id| {
                    standard_recipient_matches_normalized_user_id(
                        &needle,
...
/// Returns whether a configured recipient identifies an OpenPGP user ID or
/// the complete email address enclosed by that user ID.
///
/// The closing `>` must terminate the user ID. This keeps recipient matching
/// identical for store encryption and consumers such as Git audit history.
pub fn standard_recipient_matches_user_id(recipient: &str, user_id: &str) -> bool {
    let Some(needle) = normalized_standard_recipient_lookup(recipient) else {
        return false;
    };

    standard_recipient_matches_normalized_user_id(&needle, user_id)
}

fn standard_recipient_matches_normalized_user_id(needle: &str, user_id: &str) -> bool {
...
pub fn resolved_recipients_from_contents<'a>(
    contents: &str,
    key_ring: &'a HashMap<[u8; 20], Arc<Cert>>,
...
pub fn standard_recipient_file_contents(
...
pub fn read_store_recipient_file_contents(recipients_file: &Path) -> Result<String, String> {
    fs::read_to_string(recipients_file).map_err(|err| err.to_string())
}

pub fn private_key_requirement_from_contents(
    contents: &str,
...
pub const fn effective_private_key_requirement(
...
pub fn required_private_key_fingerprints_from_contents(
    contents: &str,
...
pub fn encryption_context_fingerprint_from_contents(
    contents: &str,
    key_ring: &HashMap<[u8; 20], Arc<Cert>>,
...
fn private_key_use_priority(fingerprint: &str) -> PrivateKeyUsePriority {
    match ensure_ripasso_private_key_is_ready(fingerprint) {
        ...
        Err(PrivateKeyReadinessError::Locked(_)) => PrivateKeyUsePriority::Unlockable,
...
pub fn private_key_requirement_for_label(
    store_root: &str,
    label: &str,
...
fn private_key_is_openable_with_unlock(fingerprint: &str) -> bool {
    matches!(
        ...
        Ok(()) | Err(PrivateKeyReadinessError::Locked(_))
    )
}

pub fn decryption_candidate_fingerprints_for_entry(
    store_root: &str,
    label: &str,
) -> Result<Vec<String>, String> {
    if matches!(
...
mod tests {
    ...
    #[test]
    fn standard_recipient_email_requires_a_complete_user_id() {
        assert!(super::standard_recipient_matches_user_id(
            "alice@example.com",
            "Alice Example <alice@example.com>"
        ));
        assert!(!super::standard_recipient_matches_user_id(
            "alice@example.com",
            "Alice Example <alice@example.com> trailing"
        ));
    }
