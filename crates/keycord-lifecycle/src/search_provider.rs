...
impl SearchProviderEntry {
    pub fn from_label(store_path: impl Into<String>, label: impl Into<String>) -> Self {
...
#[derive(Clone, Copy)]
pub struct SearchProviderPorts {
    pub list_entries: fn() -> Vec<SearchProviderEntry>,
    pub store_labels: fn() -> HashMap<String, String>,
}

pub fn is_search_provider_command(args: &[OsString]) -> bool {
    args.get(1).is_some_and(|arg| arg == "--search-provider")
}

pub fn run(config: SearchProviderConfig, ports: SearchProviderPorts) -> ExitCode {
    ...
    let service = Rc::new(SearchProviderService::new(interface_info, config, ports));
    ...
    log_info(format!(
        "Search provider listening on {}{}.",
        config.bus_name, config.object_path
    ));
    main_loop.run();
...
impl SearchProviderService {
    ...
    fn register(&self, connection: &DBusConnection) -> Result<(), glib::Error> {
        let _registration_id = connection
            .register_object(self.config.object_path, &self.interface_info)
            .method_call({
                ...
                move |_connection,
                    ...
                    match method_name {
                        "GetInitialResultSet" => {
                            invocation
                                .return_result(handle_get_initial_result_set(&parameters, ports));
                        }
                        "GetSubsearchResultSet" => {
                            invocation
                                .return_result(handle_get_subsearch_result_set(&parameters, ports));
                        }
                        "GetResultMetas" => {
                        ...
                        "ActivateResult" => {
                            invocation.return_result(handle_activate_result(&parameters, ports));
                        }
                        "LaunchSearch" => {
...
fn handle_get_initial_result_set(
    ...
    Ok(Some((search_result_ids(&terms, ports),).to_variant()))
}
...
fn handle_get_result_metas(
    ...
    let store_labels = (ports.store_labels)();
    let metas = identifiers
        .into_iter()
        .map(|identifier| {
            meta_for_identifier(&identifier, &store_labels, config, ports)
                .unwrap_or_else(|| fallback_meta(&identifier, config))
        })
        .collect::<Vec<_>>();
...
fn handle_activate_result(
    ...
    match decode_result_target(&identifier, ports) {
...
fn meta_for_identifier(
    ...
    let entry = decode_result_id(identifier, ports)?;
    ...
    meta.insert("gicon".to_string(), config.app_id.to_variant());
    Some(meta)
}

fn fallback_meta(identifier: &str, config: SearchProviderConfig) -> HashMap<String, Variant> {
    let mut meta = HashMap::new();
    meta.insert("id".to_string(), identifier.to_variant());
...
fn decode_result_target(identifier: &str, ports: SearchProviderPorts) -> Option<(String, String)> {
    let entry = decode_result_id(identifier, ports)?;
    Some((entry.store_path, entry.label))
}

fn encode_result_id(entry: &SearchProviderEntry) -> String {
...
fn decode_result_id(identifier: &str, ports: SearchProviderPorts) -> Option<SearchProviderEntry> {
...
fn search_provider_entries(
    terms: &[String],
    limit: usize,
    ports: SearchProviderPorts,
) -> Vec<SearchProviderEntry> {
    ...
    for entry in (ports.list_entries)() {
...
fn search_result_ids(terms: &[String], ports: SearchProviderPorts) -> Vec<String> {
    search_provider_entries(terms, SEARCH_PROVIDER_RESULT_LIMIT, ports)
        .into_iter()
...
mod tests {
    use super::{
        ...
        SearchProviderEntry, SearchProviderPorts,
    };
    use std::collections::HashMap;

    fn no_entries() -> Vec<SearchProviderEntry> {
        Vec::new()
    }

    fn no_store_labels() -> HashMap<String, String> {
        HashMap::new()
    }

    fn test_ports() -> SearchProviderPorts {
        SearchProviderPorts {
            list_entries: no_entries,
            store_labels: no_store_labels,
        }
    }

    #[test]
    fn result_ids_are_opaque_hashes() {
        let entry = SearchProviderEntry::from_label("/tmp/store", "work/alice/github");
        ...
        assert_eq!(decode_result_id(&identifier, test_ports()), None);
    }

    #[test]
    fn invalid_result_ids_are_rejected() {
        assert_eq!(decode_result_id("", test_ports()), None);
        assert_eq!(decode_result_id("/tmp/store", test_ports()), None);
        assert_eq!(decode_result_id("xyz", test_ports()), None);
    }
