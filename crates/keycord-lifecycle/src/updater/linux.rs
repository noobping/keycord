...
use crate::setup::{
    configured as install_config, install_locally, installed_local_binary_path,
...
pub fn supports_updater() -> bool {
    is_current_executable_installed_locally(install_config())
}
...
fn run_auto_install(cleanup_dir: &Path) -> glib::ExitCode {
    (install_config().initialize_i18n)();
...
fn register_auto_install_resources() -> Result<(), String> {
    (install_config().register_resources)()
}
...
fn auto_install_update(cleanup_dir: &Path) -> Result<(), String> {
    ...
    install_locally(install_config())
...
fn restart_installed_app() {
    let Some(installed_exe) = installed_local_binary_path(install_config()) else {
...
fn linux_release_asset_name(tag_name: &str, arch: &str) -> String {
    format!("{PRODUCT_NAME}-{tag_name}.{arch}")
}
...
fn update_staging_root() -> UpdateStagingRoot {
    if let Some(base) = dirs_next::cache_dir().or_else(dirs_next::data_local_dir) {
        UpdateStagingRoot {
            path: base.join(PRODUCT_NAME).join("updates"),
            needs_creation: true,
        }
    } else {
        UpdateStagingRoot {
...
mod tests {
    ...
    fn auto_install_registers_embedded_resources() {
        crate::setup::configure(crate::setup::test_install_config());
        register_auto_install_resources().expect("register auto-install resources");
        let icon_resource_path = format!(
            "{}/scalable/apps/{}.svg",
            crate::setup::configured().resource_id,
            crate::setup::configured().app_id,
        );
        assert!(icon_resource_path.ends_with("/scalable/apps/io.github.noobping.keycord.svg"));
    }
