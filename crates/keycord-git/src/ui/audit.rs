use crate::{
    audit_available, audit_unverified_reason_message, discover_store_git_audit_catalog,
    git_command_available, has_git_repository, StoreGitAuditBranchRef, StoreGitAuditCatalog,
...
use adw::gtk::{
    Align, Box as GtkBox, Button, Grid, Image, Label, MenuButton, Orientation, Popover,
    ScrolledWindow, SearchEntry, Spinner, Stack, Widget,
};
use adw::prelude::*;
use adw::{
    ActionRow, ExpanderRow, NavigationPage, NavigationView, PreferencesGroup, StatusPage, Toast,
...
use keycord_shell::navigation::{
    navigation_stack_contains_page, reveal_navigation_page, show_secondary_page_chrome,
    visible_navigation_page_is, HasWindowChrome, PagePresentation, WindowChrome,
};
use keycord_shell::ui::{
    clear_box_children, gtk_safe_text, next_ui_generation, set_action_row_enabled,
    set_action_row_suffix_loading,
};
...
pub fn git_audit_page_presentation() -> PagePresentation {
    PagePresentation::secondary(AUDIT_TITLE, AUDIT_SUBTITLE, false).with_find_visible(true)
}
...
pub const fn audit_tool_cache_should_clear(
...
pub type LoadAuditCommitPage = Arc<
    dyn Fn(String, String, bool, usize) -> Result<StoreGitAuditCommitPage, String> + Send + Sync,
>;
pub type StoreLabelMap = Rc<dyn Fn(&[String]) -> HashMap<String, String>>;
pub type StoreLabelForPath = Rc<dyn Fn(&str, &HashMap<String, String>) -> String>;

#[derive(Clone)]
pub struct GitAuditPreferencesPorts {
    ...
    pub included_branches: Rc<dyn Fn() -> Option<Vec<String>>>,
    pub set_included_branches: Rc<dyn Fn(Vec<String>) -> Result<(), String>>,
    pub use_commit_history_recipients: Rc<dyn Fn() -> bool>,
}

#[derive(Clone)]
pub struct GitAuditPagePorts {
    pub preferences: GitAuditPreferencesPorts,
    pub load_commit_page: LoadAuditCommitPage,
    pub store_label_map: StoreLabelMap,
    pub store_label_for_path: StoreLabelForPath,
    pub apply_entry_store_filter: Rc<dyn Fn(BTreeSet<String>)>,
}

#[derive(Clone)]
pub struct GitAuditWindowNavigation {
...
impl HasWindowChrome for GitAuditWindowNavigation {
...
pub struct GitAuditPageWidgets<'a> {
    pub navigation: GitAuditWindowNavigation,
    pub overlay: &'a ToastOverlay,
    pub tool_row: &'a ActionRow,
    pub tool_suffix_stack: &'a Stack,
    pub tool_suffix_arrow: &'a Image,
    pub tool_spinner: &'a Spinner,
...
pub struct GitAuditPageState {
    ...
    ports: GitAuditPagePorts,
}

impl GitAuditPageState {
    pub fn new(widgets: GitAuditPageWidgets<'_>, ports: GitAuditPagePorts) -> Self {
        let state = Self {
            navigation: widgets.navigation,
            overlay: widgets.overlay.clone(),
            audit_row: widgets.tool_row.clone(),
            audit_suffix_stack: widgets.tool_suffix_stack.clone(),
            audit_suffix_arrow: widgets.tool_suffix_arrow.clone(),
            audit_spinner: widgets.tool_spinner.clone(),
            page: widgets.page.clone(),
            search_entry: widgets.search_entry.clone(),
            stack: widgets.stack.clone(),
            audit_status: widgets.status.clone(),
            scrolled: widgets.scrolled.clone(),
            content: widgets.content.clone(),
            filter_button: widgets.filter_button.clone(),
            filter_popover: widgets.filter_popover.clone(),
            filter_store_box: widgets.filter_store_box.clone(),
            filter_branch_box: widgets.filter_branch_box.clone(),
        ...
        state
            .audit_row
            .connect_activated(move |_| activate_state.prepare_audit_page());
        let search_state = state.clone();
        state
            .search_entry
            .connect_search_changed(move |_| search_state.render_audit_page());
        state
    }
    ...
    pub fn is_in_stack(&self) -> bool {
        navigation_stack_contains_page(&self.navigation.nav, &self.page)
    }
}
...
impl GitAuditPageState {
    pub fn prepare_audit_page(&self) {
        if !audit_available() || !git_command_available() {
            return;
        }

        let presentation = git_audit_page_presentation();
        ...
        reveal_navigation_page(&self.navigation.nav, &self.page);

        if self.audit.catalog.borrow().is_none() && !self.audit.loading_catalog.get() {
            self.start_audit_catalog_load();
        } else {
            self.sync_filter_button();
            self.render_audit_page();
        }
    }

    pub fn sync_tool_row(&self) {
        let supported = audit_available();
        self.audit_row.set_visible(supported);
        if !supported {
            return;
        }

        let availability = audit_git_runtime_availability(&(self.ports.preferences.store_roots)());
        self.audit_row
            .set_subtitle(&localized_text(audit_row_subtitle(availability)));
        set_action_row_enabled(
            &self.audit_row,
            matches!(availability, AuditGitRuntimeAvailability::Available),
        );
        set_action_row_suffix_loading(
            &self.audit_suffix_stack,
            &self.audit_suffix_arrow,
            &self.audit_spinner,
    ...
    pub fn clear_transient_state(&self) {
        self.audit
            .generation
            .set(next_ui_generation(self.audit.generation.get()));
        self.audit.loading_catalog.set(false);
        *self.audit.catalog.borrow_mut() = None;
        *self.audit.error.borrow_mut() = None;
        self.audit.store_labels.borrow_mut().clear();
        self.audit.branches.borrow_mut().clear();
        clear_box_children(&self.content);
        clear_box_children(&self.filter_store_box);
        clear_box_children(&self.filter_branch_box);
        self.reset_audit_view();
        self.set_audit_status(AUDIT_TITLE, AUDIT_NO_STORES_SUBTITLE);
        self.stack.set_visible_child(&self.audit_status);
        self.sync_filter_button();
        self.sync_tool_row();
    }

    pub fn has_transient_state(&self) -> bool {
        self.audit.loading_catalog.get()
            || self.audit.catalog.borrow().is_some()
            || !self.audit.branches.borrow().is_empty()
            || self.audit.error.borrow().is_some()
    }

    pub fn sync_filter_button(&self) {
        let visible = audit_available()
            && visible_navigation_page_is(&self.navigation.nav, &self.page)
            && self
                .audit
                .catalog
        ...
        self.filter_button.set_visible(visible);
        self.filter_button.set_sensitive(
            visible && !self.audit.loading_catalog.get() && self.audit.catalog.borrow().is_some(),
        );
        if !visible {
    ...
    fn start_audit_catalog_load(&self) {
        let generation = next_ui_generation(self.audit.generation.get());
        self.audit.generation.set(generation);
        self.audit.loading_catalog.set(true);
        *self.audit.catalog.borrow_mut() = None;
        *self.audit.error.borrow_mut() = None;
        self.audit.store_labels.borrow_mut().clear();
        self.audit.branches.borrow_mut().clear();
        ...
        let store_roots = (self.ports.preferences.store_roots)();
    ...
    fn apply_audit_catalog(&self, generation: u64, result: Result<StoreGitAuditCatalog, String>) {
        if generation != self.audit.generation.get() {
            return;
        }
        ...
        match result {
            Ok(catalog) => {
                *self.audit.error.borrow_mut() = None;
                self.audit
                    .store_labels
                    .replace((self.ports.store_label_map)(
                        &catalog
                            .stores
                            .iter()
                ...
                *self.audit.catalog.borrow_mut() = Some(catalog);
            }
            Err(err) => {
                *self.audit.catalog.borrow_mut() = None;
                *self.audit.error.borrow_mut() = Some(err);
            }
        }
    ...
    fn handle_audit_catalog_disconnect(&self, generation: u64) {
        ...
        self.audit.loading_catalog.set(false);
        *self.audit.catalog.borrow_mut() = None;
        *self.audit.error.borrow_mut() = Some("Audit stopped unexpectedly.".to_string());
        self.render_audit_filter_controls();
        self.render_audit_page();
        self.sync_tool_row();
    }

    pub fn render_audit_page(&self) {
        self.sync_filter_button();
        clear_box_children(&self.content);
        let query = self.audit_search_query();

        if self.audit.loading_catalog.get() {
            self.set_audit_status(AUDIT_LOADING_TITLE, AUDIT_LOADING_SUBTITLE);
            self.stack.set_visible_child(&self.audit_status);
            return;
        }

        if let Some(err) = self.audit.error.borrow().clone() {
            self.set_audit_status_text(AUDIT_ERROR_TITLE, &err);
            self.stack.set_visible_child(&self.audit_status);
            return;
        }

        let Some(catalog) = self.audit.catalog.borrow().clone() else {
            self.set_audit_status(AUDIT_TITLE, AUDIT_NO_STORES_SUBTITLE);
            self.stack.set_visible_child(&self.audit_status);
            return;
        };
        ...
        for store in &catalog.stores {
            ...
            let store_label = (self.ports.store_label_for_path)(
                &store.store_root,
                &self.audit.store_labels.borrow(),
            );
    ...
    fn render_audit_filter_controls(&self) {
        ...
        for store in &catalog.stores {
            ...
            let label =
                (self.ports.store_label_for_path)(&store_root, &self.audit.store_labels.borrow());
            ...
            self.filter_store_box.append(&toggle);
        }
        ...
        for branch_name in audit_available_branch_names(&catalog) {
            ...
            self.filter_branch_box.append(&toggle);
        }
    ...
    fn update_audit_store_filter(&self, store_root: &str, active: bool) {
        ...
        if let Err(err) =
            (self.ports.preferences.set_included_store_roots)(included.iter().cloned().collect())
        {
            self.handle_audit_filter_save_error("store", &err);
            self.render_audit_filter_controls();
            return;
        }
        (self.ports.apply_entry_store_filter)(included);
        self.render_audit_filter_controls();
        self.render_audit_page();
    }
    ...
    fn update_audit_branch_filter(&self, branch_name: &str, active: bool) {
        ...
        if let Err(err) =
            (self.ports.preferences.set_included_branches)(included.iter().cloned().collect())
        {
    ...
    fn included_store_filter_values(&self) -> BTreeSet<String> {
        let available = (self.ports.preferences.store_roots)()
            .into_iter()
            .collect::<BTreeSet<_>>();
        let stored = (self.ports.preferences.included_store_roots)()
        ...
        if stored.as_ref().is_some_and(|stored| stored != &included) {
            if let Err(err) = (self.ports.preferences.set_included_store_roots)(
    ...
    fn included_audit_branch_filter_values(
        ...
        let stored = (self.ports.preferences.included_branches)()
    ...
    fn handle_audit_filter_save_error(&self, filter_kind: &str, err: &str) {
        log_error(format!(
    ...
    fn audit_search_query(&self) -> String {
        audit_search_query(self.search_entry.text().as_str())
    }
    ...
    fn populate_audit_branch_row(
        ...
        let store_label =
            (self.ports.store_label_for_path)(&key.store_root, &self.audit.store_labels.borrow());
    ...
    fn start_audit_branch_load(&self, key: &AuditBranchKey, page: usize, append: bool) {
        let runtime = self.audit_branch_state(key);
        let generation = next_ui_generation(runtime.generation.get());
        ...
        *runtime.error.borrow_mut() = None;
        ...
        let use_history = (self.ports.preferences.use_commit_history_recipients)();
        let load_commit_page = self.ports.load_commit_page.clone();
        let state_for_result = self.clone();
        let state_for_disconnect = self.clone();
        let key_for_result = key.clone();
        let key_for_disconnect = key.clone();
        spawn_result_task(
            move || load_commit_page(store_root, full_ref, use_history, page),
            move |result| {
    ...
    fn audit_branch_state(&self, key: &AuditBranchKey) -> Rc<AuditBranchState> {
        if let Some(existing) = self.audit.branches.borrow().get(key).cloned() {
            return existing;
        }
    ...
    fn existing_audit_branch_state(&self, key: &AuditBranchKey) -> Option<Rc<AuditBranchState>> {
        self.audit.branches.borrow().get(key).cloned()
    }

    fn set_audit_status(&self, title: &str, description: &str) {
        self.audit_status.set_title(&localized_text(title));
        self.audit_status
...
mod tests {
    use super::{
        ...
        audit_commit_matches_query, audit_search_query, audit_tool_cache_should_clear,
    ...
    fn audit_commit_query_matches_commit_details() {
        let commit = StoreGitAuditCommit {
            ...
            changed_paths: vec![crate::StoreGitAuditPathChange {
                status: "M".to_string(),
