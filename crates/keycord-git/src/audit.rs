use super::audit_types::{
    StoreGitAuditBranchRef, StoreGitAuditCatalog, StoreGitAuditCommit, StoreGitAuditCommitPage,
    StoreGitAuditPathChange, StoreGitAuditStore, StoreGitAuditUnverifiedReason,
...
use super::command::{
    git_command_error, run_store_git_command, run_store_git_command_with_input,
...
/// Root-owned lookups needed to verify commit signatures against store recipients.
///
/// Git owns the audit algorithm, while key discovery and recipient parsing remain
/// in their respective subject crates until those APIs are extracted.
#[derive(Clone, Copy)]
pub struct StoreGitAuditPorts {
    pub available_standard_public_certs: fn() -> Result<Vec<Cert>, String>,
    pub available_host_public_certs: fn() -> Result<Vec<Cert>, String>,
    pub parse_standard_recipients: fn(&str) -> Vec<String>,
    pub normalize_standard_recipient: fn(&str) -> String,
    pub standard_recipient_matches_user_id: fn(&str, &str) -> bool,
    pub host_key_sync_enabled: fn() -> bool,
}
...
pub fn load_store_git_audit_commit_page_with_ports(
    store_root: &str,
    full_ref: &str,
    use_commit_history_recipients: bool,
    page: usize,
    ports: StoreGitAuditPorts,
) -> Result<StoreGitAuditCommitPage, String> {
    let all_certs = audit_available_standard_public_certs(ports)?;
    let branch_tip_context = load_tree_recipient_context(store_root, full_ref, &all_certs, ports)?;
    ...
    let host_signature_summaries =
        read_host_git_signature_summaries_if_enabled(store_root, &oids, ports);
...
fn audit_available_standard_public_certs(ports: StoreGitAuditPorts) -> Result<Vec<Cert>, String> {
    let mut certs = (ports.available_standard_public_certs)()?;
    if !host_git_audit_verification_enabled(ports) {
        return Ok(certs);
    }

    match (ports.available_host_public_certs)() {
...
fn host_git_audit_verification_enabled(ports: StoreGitAuditPorts) -> bool {
    (ports.host_key_sync_enabled)() && has_host_permission()
}
...
fn read_host_git_signature_summaries_if_enabled(
    ...
    if !host_git_audit_verification_enabled(ports) || oids.is_empty() {
        return HashMap::new();
    }
...
#[expect(
    ...
    reason = "Commit verification keeps the Git and recipient evidence explicit at the audit boundary."
)]
fn verify_commit(
    store_root: &str,
    _full_ref: &str,
    ...
    if let Some(reason) = authorize_signer(branch_tip_context, &signer_fingerprint) {
        if should_retry_with_commit_history_recipients(reason, use_commit_history_recipients) {
            let historical_context =
                load_tree_recipient_context(store_root, oid, all_certs, ports)?;
...
fn load_tree_recipient_context(
    ...
    for path in recipient_paths {
        ...
        let file_name = Path::new(&path)
            ...
            .unwrap_or_default();
        if file_name == ".gpg-id" {
            standard_recipients.extend((ports.parse_standard_recipients)(&contents));
        }
    }
    ...
    let resolved_standard_fingerprints = standard_recipients
        .iter()
        .filter_map(|recipient| resolve_standard_recipient_fingerprint(recipient, certs, ports))
        .collect::<HashSet<_>>();
...
fn resolve_standard_recipient_fingerprint(
    recipient: &str,
    certs: &[Cert],
    ports: StoreGitAuditPorts,
) -> Option<String> {
    let normalized = (ports.normalize_standard_recipient)(recipient);
    ...
    let matches = certs
        ...
        .filter(|cert| {
            cert.userids().any(|user_id| {
                (ports.standard_recipient_matches_user_id)(&needle, &user_id.userid().to_string())
            })
        })
        .map(|cert| cert.fingerprint().to_hex())
        .collect::<Vec<_>>();
...
mod tests {
    use super::{
        authorize_signer, commit_signature_format, parse_audit_refs, parse_changed_path_line,
        parse_commit_object, parse_commit_summaries, parse_host_git_signature_summaries,
        resolve_standard_recipient_fingerprint, should_retry_with_commit_history_recipients,
        sort_audit_refs, CommitSignatureFormat, StoreGitAuditPorts, StoreGitAuditUnverifiedReason,
        TreeRecipientContext,
    };
    use crate::audit_unverified_reason_message;
    ...
    fn no_test_certs() -> Result<Vec<Cert>, String> {
        Ok(Vec::new())
    }

    fn no_test_recipients(_contents: &str) -> Vec<String> {
        Vec::new()
    }

    fn normalize_test_recipient(value: &str) -> String {
    ...
    fn test_ports() -> StoreGitAuditPorts {
        StoreGitAuditPorts {
            available_standard_public_certs: no_test_certs,
            available_host_public_certs: no_test_certs,
            parse_standard_recipients: no_test_recipients,
            normalize_standard_recipient: normalize_test_recipient,
            standard_recipient_matches_user_id:
                keycord_stores::integrated_recipients::standard_recipient_matches_user_id,
            host_key_sync_enabled: || false,
        }
    }
    ...
    fn commit_summary_parser_reads_fixed_fields() {
        let summaries = parse_commit_summaries(
            b"abc\0abc\0Subject\0Alice <a@example.com>\x002026-01-01T00:00:00+00:00\0Bob <b@example.com>\x002026-01-01T00:00:00+00:00\0",
        );
    ...
    fn recipient_resolution_matches_fingerprint_user_id_and_email() {
        ...
        assert_eq!(
            resolve_standard_recipient_fingerprint(&fingerprint, &certs, test_ports()),
            Some(fingerprint.clone())
        );
        assert_eq!(
            resolve_standard_recipient_fingerprint(
        ...
        assert_eq!(
            resolve_standard_recipient_fingerprint("alice@example.com", &certs, test_ports()),
            Some(fingerprint)
        );
    }
