use crate::ui::{dialog_content_shell, flat_icon_button_with_tooltip};
use adw::glib::Bytes;
...
pub fn connect_copy_and_qr_buttons_with<F, ConnectCopy>(
    copy_button: &Button,
    qr_button: &Button,
    ...
    F: Fn() -> String + 'static,
    ConnectCopy: FnOnce(&Button, &ToastOverlay, Rc<dyn Fn() -> String>),
{
    let text: Rc<dyn Fn() -> String> = Rc::new(text);
    let copy_text = text.clone();
    connect_copy(copy_button, overlay, copy_text);
