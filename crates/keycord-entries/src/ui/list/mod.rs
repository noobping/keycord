...
use keycord_shell::navigation::{
    show_primary_page_chrome, PrimaryPageActionVisibility, WindowChrome, APP_WINDOW_TITLE,
};
use keycord_shell::object_data::{cloned_data, non_null_to_string_option, set_cloned_data};
use keycord_shell::ui::{
    clear_list_box, connect_search_list_arrow_navigation, navigation_stack_is_root,
};
use keycord_stores::labels::shortened_store_label_map;
use std::cell::Cell;
...
pub fn apply_password_list_store_filter(list: &ListBox, included_store_roots: BTreeSet<String>) {
...
struct ListActionVisibility {
    add: Visibility,
    find: Visibility,
    primary_action: Visibility,
    secondary_action: Visibility,
    save: Visibility,
}
...
#[derive(Clone)]
pub struct PasswordListVisibilityState {
    show_hidden: Rc<Cell<bool>>,
    show_duplicates: Rc<Cell<bool>>,
}

impl PasswordListVisibilityState {
...
pub fn show_password_list_chrome(chrome: &WindowChrome<'_>, actions: PrimaryPageActionVisibility) {
    configure_password_save_button(chrome.save);
    show_primary_page_chrome(
        chrome,
        actions,
        APP_WINDOW_TITLE,
        super::page::PASSWORD_LIST_SUBTITLE,
    );
}
...
pub fn register_password_list_window_actions(
    ...
    let reload_after_toggle = reload.clone();
    register_window_action(window, "toggle-hidden-and-duplicates", move || {
        if !navigation_stack_is_root(&navigation) {
            return;
        }
        visibility.toggle_all();
        reload_after_toggle();
    });
    register_window_action(window, "reload-password-list", move || reload());
}
...
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum PasswordListActionRowKind {
    NewPassword,
    ClearSearch,
}
...
const fn list_action_visibility(context: ListActionContext) -> ListActionVisibility {
    if matches!(context.actions, ListActionsMode::Hidden) {
        return ListActionVisibility {
            ...
            primary_action: Visibility::Hidden,
            secondary_action: Visibility::Hidden,
    ...
    ListActionVisibility {
        ...
        primary_action: if should_show_root_git_button(context) {
            Visibility::Visible
        } else {
            Visibility::Hidden
        },
        secondary_action: if should_show_root_store_button(context) {
            Visibility::Visible
        } else {
            Visibility::Hidden
        },
...
pub fn load_passwords_async(
    ...
    if should_show_list_actions() {
        actions.primary_action.set_visible(false);
        actions.secondary_action.set_visible(false);
    ...
    let collect_items = ports.backend.collect_items.clone();
    let is_readable = ports.backend.is_readable.clone();
    let result_ports = ports.clone();
    spawn_result_task(
        move || {
            let mut items = collect_items(collect_items_options(show_hidden, show_duplicates))
                .into_iter()
                .map(|item| {
                    let label = item.label();
                    let readable = is_readable(item.store_path.clone(), label);
                    (item, readable)
                })
                .collect::<Vec<_>>();
...
pub struct PasswordListSearchWidgets<'a> {
    pub list: &'a ListBox,
    pub search_entry: &'a SearchEntry,
    pub header_focus_target: &'a Widget,
    pub placeholder_stack: &'a adw::gtk::Stack,
    pub placeholder_status: &'a adw::StatusPage,
    pub placeholder_spinner: &'a adw::gtk::Spinner,
    pub list_view: &'a adw::gtk::ScrolledWindow,
}

pub fn setup_search_filter(widgets: PasswordListSearchWidgets<'_>, ports: &EntryListUiPorts) {
    let PasswordListSearchWidgets {
        list,
        search_entry,
        header_focus_target,
        placeholder_stack,
    ...
    let controller = SearchFilterController::new(
        (ports.preferences.included_store_roots)().map(|roots| roots.into_iter().collect()),
        ports.read_entry_for_search(),
    );
    ...
    let sort_mode = ports.preferences.sort_mode.clone();
    search_entry.connect_search_changed(move |entry| {
        ...
        if was_empty != is_empty
            && password_list_search_change_requires_reload(sort_mode(), was_empty, is_empty)
            && list_for_entry
...
pub fn connect_password_list_search_visibility(
    find_button: &Button,
...
pub fn toggle_password_list_search(
    find_button: &Button,
    search_entry: &SearchEntry,
    list: &ListBox,
) {
...
pub fn apply_password_list_startup_query(
    startup_query: Option<String>,
    search_entry: &SearchEntry,
    list: &ListBox,
) {
    if let Some(query) = startup_query.filter(|query| !query.is_empty()) {
        search_entry.set_visible(true);
...
pub fn password_list_row_action_kind(row: &ListBoxRow) -> Option<PasswordListActionRowKind> {
...
pub fn toggle_password_list_folder_row(list: &ListBox, row: &ListBoxRow) -> bool {
...
fn update_list_actions(actions: &PasswordListActions, context: ListActionContext) {
    ...
    actions
        .primary_action
        .set_visible(visibility.primary_action.is_visible());
    actions
        .secondary_action
        .set_visible(visibility.secondary_action.is_visible());
}

fn prune_missing_store_dirs(ports: &EntryListUiPorts) {
    if let Err(err) = (ports.preferences.prune_missing_stores)() {
        log_error(format!("Failed to remove missing password stores: {err}"));
    }
}

fn log_store_git_state(ports: &EntryListUiPorts) {
    let stores = (ports.preferences.store_roots)();
    let summary = if stores.is_empty() {
        ...
        for store in stores {
            lines.push((ports.store_git_summary)(store));
        }
        lines.join("\n")
    };
...
pub fn password_list_render_generation(list: &ListBox) -> Option<u64> {
...
#[cfg(test)]
mod tests {
    use super::row::SelectedPasswordRowAction;
    use super::{
        ...
        should_show_root_store_button, sort_password_list_items, toggled_list_visibility,
        GitAvailability, ListActionContext, ListActionVisibility, ListActionsMode, ListContents,
    ...
    fn expected_root_action_visibility_for_empty_store_setup() -> ListActionVisibility {
        ListActionVisibility {
            add: Visibility::Hidden,
            find: Visibility::Hidden,
            primary_action: Visibility::Visible,
            secondary_action: Visibility::Visible,
            save: Visibility::Hidden,
        }
    }
    ...
    fn list_actions_hide_git_when_runtime_git_is_unavailable() {
        assert_eq!(
            ...
            ListActionVisibility {
                ...
                find: Visibility::Hidden,
                primary_action: Visibility::Hidden,
                secondary_action: if expected_store_visibility_without_git() {
                    Visibility::Visible
                } else {
                    Visibility::Hidden
                },
