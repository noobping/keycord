...
pub(super) use crate::search::SearchRowFieldIndexState;
use crate::search::{parse_search_query, row_matches_query, SearchQuery};
...
impl SearchFilterController {
    ...
    pub(super) fn refresh_row_visibility(&self, list: &ListBox) {
        ...
        for_each_list_box_row(list, |row| match password_list_row_action_kind(&row) {
    ...
    pub(super) fn start_indexing_if_needed(&self, list: &ListBox) {
        ...
        let read_password_entry = self.state.read_password_entry.clone();
        spawn_result_task(
            move || build_search_index_batch(generation, requests, &read_password_entry),
