...
use super::{
    refresh_password_list_filter, EntryListUiPorts, PasswordListActionRowKind,
    PASSWORD_LIST_ROW_DEPTH_KEY, PASSWORD_LIST_ROW_EXPANDED_KEY, PASSWORD_LIST_ROW_KIND_ENTRY,
    PASSWORD_LIST_ROW_KIND_FOLDER, PASSWORD_LIST_ROW_KIND_KEY, PASSWORD_LIST_ROW_STORE_PATH_KEY,
};
...
use crate::undo::{
    move_entry_between_stores_action, rename_entry_action, unavailable_undo_action,
    unavailable_undo_message, UndoAction, UndoError,
};
...
use keycord_shell::object_data::{cloned_data, set_cloned_data, set_string_data};
use keycord_shell::qr_code::copy_qr_button_group;
...
use keycord_stores::labels::{shortened_store_label_for_path, shortened_store_labels};
...
fn connect_text_edit_actions(
    ...
    text_edit_row.connect_apply(move |row| {
        ...
        match (state.ports.backend.rename_entry)(
...
fn enter_store_edit_mode(state: &PasswordRowState, overlay: &ToastOverlay) {
    let stores = (state.ports.preferences.store_roots)();
    if stores.len() < 2 {
...
fn connect_store_move_actions(
    ...
    apply_button.connect_clicked(move |_| {
        ...
        let move_entry_to_store = state.ports.backend.move_entry_to_store.clone();
        spawn_result_task(
...
fn delete_current_entry(state: &PasswordRowState, list: &ListBox, overlay: &ToastOverlay) {
    ...
    let overlay = overlay.clone();
    let overlay_for_disconnect = overlay.clone();
    let delete_entry_with_undo = state.ports.backend.delete_entry_with_undo.clone();
    spawn_result_task(
        move || delete_entry_with_undo(entry),
        move |result| match result {
            Ok(undo_action) => {
...
fn open_entry_in_new_window(state: &PasswordRowState, overlay: &ToastOverlay) {
    ...
    let entry = state.item.borrow().clone();
    match (state.ports.open_entry_window)(&app, entry) {
...
fn push_row_undo_action(widget: &impl IsA<adw::gtk::Widget>, readable: bool, action: UndoAction) {
    if readable {
